...
def build_development_content(places: list[DiningPlace], source: Path, output: Path) -> None:
    if output.exists():
    ...
    def serialize_photo(photo: Photo) -> dict[str, str | None]:
        nonlocal media_index
        media_index += 1
        public_filename = f"photo-{media_index}{photo.source.suffix.lower()}"
        shutil.copyfile(photo.source, media_directory / public_filename)
        return {
            "src": f"/media/{public_filename}",
            "title": photo.title,
            "kind": photo.kind,
    ...
    def latest_photo(photos: list[dict[str, str | None]]) -> dict[str, str | None]:
        return max(
            photos,
            key=lambda photo: (
    ...
    def cover_title(photo: dict[str, str | None]) -> str | None:
        labels = {
            "storefront": "门面照片",
            "menu": "菜单照片",
            "photo": "普通照片",
    ...
    def serialize_dishes(
        ...
        records_by_name: dict[str, list[dict[str, str | None]]] = {}
        for photo in photos:
            if photo["kind"] == "dish":
...
def run_dev(source: Path, host: str, port: int) -> int:
    ...
    build_development_content(places, source, DEVELOPMENT_OUTPUT)
